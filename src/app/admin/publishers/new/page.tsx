...
export default async function NewPublisherPage() {
  ...
  const [companies, profiles] = await Promise.all([getCompanyOptions(), getProfileOptions()]);
  return (
    <main className="mx-auto w-full max-w-2xl p-6">
      <h1 className="mb-6 text-xl font-semibold text-slate-900">New publisher</h1>
      <PublisherForm action={createPublisher} companies={companies} profiles={profiles} submitLabel="Create publisher" />
    </main>
  );
}
