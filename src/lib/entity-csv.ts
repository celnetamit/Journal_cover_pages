...
export const ENTITY_SPECS: Record<EntityKey, Spec> = {
  ...
  publishers: {
    ...
    headers: ["name", "logoUrl", "about", "email", "phone", "website", "company"],
    async exportRows() {
      const rows = await prisma.publisher.findMany({ orderBy: { name: "asc" }, include: { company: true } });
      return rows.map((p) => [p.name, p.logoUrl ?? "", p.about ?? "", p.email ?? "", p.phone ?? "", p.website ?? "", p.company?.name ?? ""]);
    },
    async importRecords(records) {
      const out = emptySummary();
      ...
      for (const [i, r] of records.entries()) {
        ...
        const data = { name, logoUrl: nul(r.logoUrl), about: nul(r.about), email: nul(r.email), phone: nul(r.phone), website: nul(r.website), companyId };
