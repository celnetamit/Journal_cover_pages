...
export const ENTITY_SPECS: Record<EntityKey, Spec> = {
  profiles: {
    ...
    headers: ["name", "email", "designation", "department", "affiliation", "address", "photoUrl", "biography"],
    async exportRows() {
      const rows = await prisma.profile.findMany({ orderBy: { name: "asc" } });
      return rows.map((p) => [p.name, p.email ?? "", p.designation ?? "", p.department ?? "", p.affiliation ?? "", p.address ?? "", p.photoUrl ?? "", p.biography ?? ""]);
    },
  ...
  companies: {
    ...
    headers: ["name", "email", "phone", "website", "cin", "gst", "registeredAddress", "salesAddress", "bankAccountName", "bankAccountNo", "bankIfsc", "bankName", "bankBranch", "bankSwift", "director"],
    async exportRows() {
      const rows = await prisma.company.findMany({ orderBy: { name: "asc" }, include: { director: true } });
      return rows.map((c) => [c.name, c.email ?? "", c.phone ?? "", c.website ?? "", c.cin ?? "", c.gst ?? "", c.registeredAddress ?? "", c.salesAddress ?? "", c.bankAccountName ?? "", c.bankAccountNo ?? "", c.bankIfsc ?? "", c.bankName ?? "", c.bankBranch ?? "", c.bankSwift ?? "", c.director?.email || c.director?.name || ""]);
    },
    async importRecords(records) {
      const out = emptySummary();
      const resolveProfile = await profileResolver();
  ...
  journals: {
    ...
    headers: ["slug", "name", "abbreviation", "shortName", "website", "issnPrint", "issnOnline", "sjif", "icv", "doi", "impactFactor", "frequency", "issuesPerYear", "about", "focusScope", "keywords", "indexing", "domain", "publisher", "manager"],
    async exportRows() {
      ...
      return rows.map((j) => [
        ...
        j.about ?? "", list(j.focusScope), list(j.keywords), list(j.indexing),
    ...
    async importRecords(records) {
      ...
      for (const [i, r] of records.entries()) {
        ...
        const data = {
          ...
          focusScope: unlist(r.focusScope), keywords: unlist(r.keywords), indexing: unlist(r.indexing),
